...
export default function NosotrosPage() {
  return <><SiteHeader /><main><PageIntro eyebrow="Nosotros" title="Pocas manos. Mucho oficio." copy="Somos estrategas, diseñadores, desarrolladores y creadores trabajando como un único equipo." /><section className="mx-auto max-w-7xl px-6 py-20 lg:px-10"><div className="grid gap-4 md:grid-cols-3">{VALUES.map(([n, title, copy]) => <article key={n} className="swira-panel bg-white p-8 md:p-10"><p className="font-heading text-xl font-bold text-brand">{n}</p><h2 className="mt-16 font-heading text-3xl font-bold">{title}</h2><p className="mt-4 leading-relaxed text-muted-foreground">{copy}</p></article>)}</div></section><Team /><FinalCta /></main><SiteFooter /></>
}
