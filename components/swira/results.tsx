...
function AnimatedMetric({ value, prefix, suffix }: { value: number; prefix: string; suffix: string }) {
  ...
  return <p ref={ref} className="font-heading text-[clamp(3.5rem,7vw,7rem)] font-extrabold leading-none tracking-[-.06em] text-brand">{prefix}{display}{suffix}</p>
}
    ...
    <section data-theme="dark" className="swira-atmosphere-ink py-24 text-white md:py-32">
      <div className="swira-container">
        ...
        <div className="mt-14 grid gap-4 md:grid-cols-3">
          {RESULTS.map((result) => <article key={result.client} className="min-w-0 rounded-[var(--swira-card-radius)] border border-white/15 bg-white/5 px-6 py-9"><AnimatedMetric value={result.value} prefix={result.prefix} suffix={result.suffix} /><p className="mt-5 max-w-xs text-lg font-medium leading-snug">{result.description}</p><p className="mt-7 text-sm text-white/45">{result.client}</p></article>)}
        </div>
      </div>
    </section>
  )
