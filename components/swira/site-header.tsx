  ...
  return (
    <header data-compact={compact && !open} className="site-header pointer-events-none fixed inset-x-0 top-0 z-50 px-4 pt-4 md:px-8 md:pt-6">
      <div className="site-header-bar pointer-events-auto mx-auto flex h-16 max-w-7xl items-center justify-between rounded-full border px-5 text-ink md:h-18 md:px-7">
        ...
        <Link href="/#contacto" className="hidden items-center gap-2 rounded-full bg-ink px-5 py-3 text-xs font-semibold tracking-[.06em] text-white uppercase transition-transform hover:-translate-y-0.5 md:inline-flex">Agendar reunión <ArrowUpRight className="size-4" aria-hidden="true" /></Link>
      ...
      <AnimatePresence>
        {open ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pointer-events-auto fixed inset-0 z-50 bg-[#f7f9fc] p-4 text-ink md:hidden">
            <div className="flex h-16 items-center justify-between rounded-full border border-black/5 bg-white/90 px-5 backdrop-blur-xl">
            ...
            <nav aria-label="Navegación móvil" className="flex flex-col px-3 pt-14">
              {NAV_LINKS.map((link, index) => (
                <motion.div key={link.href} initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: reduceMotion ? 0 : .08 + index * .07 }}>
                  <Link href={link.href} onClick={() => setOpen(false)} className="block border-b border-black/10 py-5 font-heading text-5xl font-bold tracking-tight hover:text-brand">{link.label}</Link>
                </motion.div>
              ))}
              <Link href="/#contacto" onClick={() => setOpen(false)} className="mt-9 inline-flex w-fit items-center gap-2 rounded-full bg-ink px-6 py-4 font-medium text-white">Agendar reunión <ArrowUpRight className="size-5" aria-hidden="true" /></Link>
            </nav>
          </motion.div>
        ) : null}
      </AnimatePresence>
