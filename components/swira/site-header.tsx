...
export function SiteHeader() {
  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-0 z-50 px-4 pt-4 md:px-8 md:pt-6">
      <div className="pointer-events-auto mx-auto flex h-16 max-w-7xl items-center justify-between rounded-full border border-white/70 bg-white/70 px-5 shadow-[0_16px_50px_rgba(11,24,54,0.12)] backdrop-blur-xl md:h-18 md:px-7">
        ...
        <Link href="/contacto" className="hidden items-center gap-2 rounded-full bg-[#111] px-5 py-3 text-xs font-semibold tracking-[0.06em] text-white uppercase transition-transform hover:-translate-y-0.5 md:inline-flex">Agendar reunión <ArrowUpRight className="size-4" aria-hidden="true" /></Link>
      ...
      {open ? <div className="pointer-events-auto fixed inset-0 z-50 bg-white/92 p-4 backdrop-blur-2xl md:hidden">
        <div className="flex h-16 items-center justify-between rounded-full border border-border bg-white/80 px-5">
        ...
        <nav aria-label="Navegación móvil" className="flex flex-col px-3 pt-16">
          ...
          <Link href="/contacto" onClick={() => setOpen(false)} className="mt-8 inline-flex w-fit items-center gap-2 rounded-full bg-primary px-6 py-4 text-white">Agendar reunión <ArrowUpRight className="size-5" /></Link>
        </nav>
      </div> : null}
    </header>
  )
