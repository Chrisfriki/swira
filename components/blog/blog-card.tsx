...
export function BlogCard({ post, featured = false, dark = false }: { post: BlogPostSummary; featured?: boolean; dark?: boolean }) {
  return (
    <article className={dark ? 'rounded-[var(--swira-card-radius)] border border-white/10 bg-white/[.035] p-4' : featured ? 'grid gap-7 lg:grid-cols-[1.3fr_.7fr] lg:items-center' : ''}>
      <Link href={`/blog/${post.slug}`} className={`group relative block aspect-[16/10] overflow-hidden rounded-[1.25rem] border ${dark ? 'border-white/10 bg-white/5' : 'border-border bg-muted'}`}>
